...
def mac_info_explore(ass_mat, video_info, fea, cluster_num, seed):
    ...
    for mac in macs:
        ...
        fea_record_group_idx = {}

        ass_idx_list = []
        ass_fea_list = []
        for r, record in enumerate(mac_info):
            ass_i = record[2:]
            ass_idx = idx_tmp[ass_i > 0]
        ...
        acc = adjusted_mutual_info_score(video_info[np.asarray(ass_idx_list), 0][labels_fea != -1], labels_fea[labels_fea != -1])
        acc_box.append(acc)
        ...
        fea_group = {}
        for i, label_i in enumerate(labels_fea):
            if label_i == -1:
                continue
            if label_i not in fea_group:
                fea_group[label_i] = {'fea': [], 'record_id': []}
            fea_group[label_i]['fea'].append(ass_idx_list[i])
            fea_group[label_i]['record_id'].extend(fea_record_group_idx[ass_idx_list[i]])
        ...
        assert ((video_ass_matrix - video_ass_matrix.T) == 0).all()
    ...
    return output, acc_box, np.asarray(as_acc).mean()
